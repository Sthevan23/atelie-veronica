<?php
/**
 * API Verônica — MySQL Hostinger
 * Banco: u586160337_atelie_conf
 */
...
function db_or_fail(): PDO {
  try {
    ...
    json_out([
      ...
      'detail' => $e->getMessage(),
      'hint' => 'Confira api/config.local.php (usuário/senha) e se importou api/veronica_mysql.sql',
    ], 500);
  }
}
...
if ($method === 'POST') {
  ...
  if (($body['action'] ?? '') === 'login') {
    ...
    if ($stored === null) {
      json_out([
        'error' => 'Banco sem dados. Importe api/veronica_mysql.sql no phpMyAdmin.',
      ], 404);
    }

    $authEmail = strtolower(trim((string) ($stored['auth']['email'] ?? '')));
    $authEmail = preg_replace('/\s+/', '', $authEmail);
    $authPass = trim((string) ($stored['auth']['password'] ?? ''));

    if ($authEmail === '' || $authPass === '') {
      json_out([
        'error' => 'Admin não configurado. Rode api/fix_admin_login.sql no phpMyAdmin.',
      ], 401);
    }
    ...
    // Resposta leve — no celular o JSON cheio (fotos) derrubava o login
    json_out([
      'ok' => true,
      'lite' => true,
      'auth' => [
  ...
  // Fidelidade — progresso por WhatsApp (público)
  if (($body['action'] ?? '') === 'loyalty_status') {
    ...
    try {
      json_out(['ok' => true, 'loyalty' => aurora_loyalty_stats($pdo, $phone)]);
    } catch (Throwable $e) {
