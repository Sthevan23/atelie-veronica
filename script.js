/**
 * Verônica Rodrigues — site público
...
const CATEGORY_LABELS = {
  all: 'Todos',
  'cat-bolos': 'Bolos',
  'cat-copos': 'Copos da Felicidade',
  'cat-fatias': 'Fatias',
};

const FILTERS = ['all', 'cat-bolos', 'cat-copos', 'cat-fatias'];

let activeFilter = 'all';
let selectedProduct = null;
let selectedFlavors = [];

const CART_KEY = 'veronica_cart_v1';
const CUSTOMER_KEY = 'veronica_customer_v1';
const COUPON_KEY = 'veronica_coupon_v1';
const FULFILLMENT_KEY = 'veronica_fulfillment_v1';
...
function fulfillmentWhatsAppBlock(mode) {
  ...
  return (
    ...
    `Endereço: Retirada sob encomenda`
  );
}
...
const LIVE_ORIGIN = 'https://ateliêveronica.com.br';
...
function imgSrc(path) {
  ...
  // Só prefixa domínio externo se LIVE_ORIGIN estiver configurado
  if (LIVE_ORIGIN && isOfflineOrLocalPage()) {
    return `${LIVE_ORIGIN.replace(/\/$/, '')}/${clean}`;
  }
  return clean;
}
...
function buildCartWhatsAppMessage({ fullName, phone, items, fulfillment, loyalty }) {
  ...
  const storeName = (s.name || 'Ateliê Verônica Rodrigues').toUpperCase();
  ...
  if (loyalty && loyalty.eligible) {
    const gift = loyalty.gift || '1 brinde surpresa da Verônica';
    loyaltyBlock =
      `Fidelidade Verônica\n` +
...
function applySettings() {
  ...
  const placeShort = 'Encomendas personalizadas';
  const ig = s.instagram || '';
  const igUser = s.instagramUser || '';
  const mapsUrl =
  ...
  const addressShort = address
    ...
    .replace(/\s*—\s*Bairro Novo Horizonte 2.*/i, '')
    .trim() || 'Novo Horizonte 2';
  ...
  if (heroBg) {
    const banner = s.banner || 'products/foto-hero-redvelvet.jpg';
    heroBg.style.backgroundImage = `url('${imgSrc(banner)}')`;
  }
  ...
  const orderText = 'Olá, Verônica! Quero fazer um pedido 😊';
  const floatMsg = 'Olá, Verônica! Tenho uma dúvida 😊';
  ...
  if (footerTagline && s.tagline) {
    footerTagline.textContent = s.tagline;
  }

  // Título da aba = nome do domínio (atelieveronica) — corrige grafias erradas tipo Veronika/Rodriguez
  document.title = brandSiteTitle(s.name);
}

function brandSiteTitle(name) {
  let n = String(name || '').trim();
  n = n
    .replace(/Veronika/gi, 'Verônica')
    .replace(/Rodriguez/gi, 'Rodrigues');
  if (!n || !/ver[oô]nica/i.test(n)) {
    return 'Ateliê Verônica Rodrigues';
  }
  // Garante acento em "Verônica" quando veio sem
  n = n.replace(/Veronica/g, 'Verônica').replace(/veronica/g, 'verônica');
  return n;
}
...
function productCardHTML(p, { bestSeller = false } = {}) {
  const flavorsHint = Array.isArray(p.flavors) && p.flavors.length
    ? `<p class="product-card__flavor-hint">${p.flavors.every((f) => /calda/i.test(f)) ? 'Escolha 1 calda ao adicionar' : `${p.flavors.length} sabores — toque para escolher e adicionar`}</p>`
    : '';
...
function renderGallery() {
  ...
  const ig = Storage.getSettings().instagramUser || '';
...
function renderLightboxFlavors() {
  ...
  const isCalda = flavors.every((f) => /calda/i.test(String(f)));
  const titleText = isCalda
    ? (multi ? 'Escolha a calda de cada unidade *' : 'Escolha a calda *')
    : (multi ? 'Escolha o sabor de cada unidade *' : 'Escolha o sabor *');
  const unitsHint = isCalda
    ? 'Escolha só um tipo de calda por unidade.'
    : 'Pode ser o mesmo sabor ou sabores diferentes.';
  ...
  flavorsBox.innerHTML = `
    <div class="order-acc ${allDone && !multi ? '' : 'is-open'} ${allDone ? 'is-done' : ''}" id="acc-flavor">
      <button type="button" class="order-acc__head" id="acc-flavor-toggle">
        <span class="order-acc__title">${titleText}</span>
      ...
      <div class="order-acc__body"><div class="order-acc__inner">
        <p class="flavor-units__hint">${multi ? unitsHint : 'Escolha apenas 1 opção.'}</p>
...
function renderCartUI() {
  ...
  itemsEl.innerHTML = cartItems.map((item) => {
    ...
    const flavorLine = item.flavor
      ? `<p class="cart-item__meta"><strong>${/calda/i.test(item.flavor) ? 'Calda' : 'Sabor'}:</strong> ${item.flavor}</p>`
      : '';
...
function initContactForm() {
  ...
  document.getElementById('contact-form')?.addEventListener('submit', (e) => {
    ...
    const text = `Olá, Verônica! Sou ${name}.\nWhatsApp: ${phone}\n\n${message}`;
    if (ok) {
      ok.hidden = false;
...
async function boot() {
  ...
  if (!online) {
    console.info('Modo local — dados padrão Verônica Rodrigues');
  }
  ...
  // Quando ela altera no painel, o site público puxa de novo sozinho
  if (typeof Storage.startPublicPolling === 'function') {
    Storage.startPublicPolling(10000);
  }
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible' && typeof Storage.pullPublic === 'function') {
      Storage.pullPublic().catch(() => {});
    }
  });
}
