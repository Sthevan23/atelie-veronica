/**
 * admin.js — Painel Administrativo Verônica Rodrigues
...
(function guardAdminAuth() {
  ...
  // Restaura senha no celular (precisa dela pra mandar alteração pro site)
  try {
    if (typeof Storage !== 'undefined' && Storage.getAdminPassword && !Storage.getAdminPassword()) {
      const saved = localStorage.getItem('veronica_admin_password') || '';
      if (saved) Storage.setAdminPassword(saved);
    }
  } catch { /* ignore */ }
})();
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!Storage.getAdminPassword()) {
    showToast('Sessão sem senha. Entre de novo pra salvar no site.', 'error');
    setTimeout(() => { window.location.replace('login.html'); }, 1200);
    return;
  }

  await Storage.initCloud({ full: true });
  Storage.startCloudPolling(8000);
  updateSyncBadge();

  if (!Storage.isCloudEnabled()) {
    showToast('Atenção: sem nuvem. Alterações podem ficar só neste celular.', 'error');
  }
...
async function pushToSite(successMsg) {
  if (!Storage.getAdminPassword()) {
    showToast('Entre de novo no admin pra poder salvar no site.', 'error');
    return false;
  }
  const ok = await Storage.saveAsync(Storage.getAll());
  updateSyncBadge();
  if (!ok) {
    showToast('Não foi pro site. Confira a internet e tente de novo.', 'error');
    return false;
  }
...
function initLogout() {
  document.getElementById('logout-btn').addEventListener('click', (e) => {
    ...
    try { localStorage.removeItem('veronica_admin_password'); } catch { /* ignore */ }
...
const ADMIN_FALLBACK_IMG = 'products/bolo-brownie-drip.png';
...
function renderProducts() {
  ...
  tbody.innerHTML = products.map(p => `
    <tr>
      ...
      <td>${p.featured ? '<i class="fas fa-star" style="color:#FFD700"></i>' : '—'}${p.bestSeller ? ' <span class="badge badge--novo">Mais vendido</span>' : ''}${p.promoActive ? ' <span class="badge badge--novo">Promo</span>' : ''}${p.active === false ? ' <span class="badge">Oculto</span>' : ''}</td>
      <td>
  ...
  document.getElementById('product-form').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const data = {
        ...
        image: imageValue || 'products/bolo-brownie-drip.png',
...
async function deleteProduct(id) {
  ...
  renderDashboard();
  if (!ok) {
    showToast('Excluído aqui, mas não sincronizou no site. Tente de novo.', 'error');
    return;
  }
...
function openCategoryModal(category = null) {
  ...
  document.getElementById('category-form').addEventListener('submit', async (e) => {
    ...
    const ok = typeof Storage.saveCategoriesAsync === 'function'
      ? await Storage.saveCategoriesAsync(categories)
      : (Storage.saveCategories(categories), false);
    closeModal();
    renderCategories();
    if (!ok) {
      showToast('Salvo aqui, mas não foi pro site. Tente de novo.', 'error');
      return;
    }
...
async function deleteCategory(id) {
  ...
  renderCategories();
  if (!ok) {
    showToast('Excluída aqui, mas não sincronizou no site.', 'error');
    return;
  }
...
function openClientModal(client = null) {
  ...
  document.getElementById('client-form').addEventListener('submit', async (e) => {
    ...
    const ok = typeof Storage.saveClientsAsync === 'function'
      ? await Storage.saveClientsAsync(clients)
      : (Storage.saveClients(clients), false);
    if (!ok) {
...
function initSettings() {
  ...
  document.getElementById('settings-form').addEventListener('submit', async (e) => {
    ...
    showToast('Salvando no site…', 'success');
    const ok = typeof Storage.saveSettingsAsync === 'function'
      ? await Storage.saveSettingsAsync(payload)
      : (Storage.saveSettings(payload), false);
    updateSyncBadge();
    if (!ok) {
      showToast('Não sincronizou na nuvem. Confira a internet, saia e entre de novo no admin, e tente outra vez.', 'error');
      return;
    }
    showToast('Pronto! Já está no site (atualiza em até 10 segundos).', 'success');
  });
  ...
  document.getElementById('password-form').addEventListener('submit', async (e) => {
    ...
    const ok = typeof Storage.updatePasswordAsync === 'function'
      ? await Storage.updatePasswordAsync(current, next)
      : Storage.updatePassword(current, next);
    if (!ok) {
      showToast('Senha atual incorreta ou falha ao salvar na nuvem.', 'error');
      return;
    }
