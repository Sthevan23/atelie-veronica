...
function aurora_loyalty_stats(PDO $pdo, string $phone): array {
  ...
  // Contagem flexível: compara WhatsApp normalizado (evita falha com/sem 9 ou 55)
  $siteTotal = 0;
  try {
    $rows = $pdo->query(
      "SELECT client_whatsapp FROM orders WHERE status <> 'cancelado'"
    )->fetchAll(PDO::FETCH_COLUMN);
    foreach ($rows as $wa) {
      if (aurora_phones_equivalent($phone, (string) $wa)) {
        $siteTotal++;
      }
    }
  } catch (Throwable $e) {
  ...
  $bonus = 0;
  try {
    $clientRows = $pdo->query('SELECT phone, loyalty_bonus FROM clients')->fetchAll(PDO::FETCH_ASSOC);
    foreach ($clientRows as $row) {
      if (!aurora_phones_equivalent($phone, (string) ($row['phone'] ?? ''))) continue;
      $bonus = max($bonus, (int) ($row['loyalty_bonus'] ?? 0));
    }
  } catch (Throwable $e) {
    // Coluna loyalty_bonus pode ainda não existir
    $bonus = 0;
  }
