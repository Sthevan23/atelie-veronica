...
(function () {
  ...
  function paintLoyalty(loyalty) {
    ...
    if (msg) {
      if (loyalty.eligible) {
        msg.textContent = `Brinde liberado: ${loyalty.gift || '1 brinde surpresa da Verônica'}!`;
  ...
  async function checkout() {
    ...
    if (!saved?.ok) {
      ...
      if (error) {
        error.textContent = saved?.error || 'Não deu para registrar. Tente de novo.';
        error.hidden = false;
      }
      return;
    }
  ...
  async function boot() {
    ...
    if (typeof Storage.startPublicPolling === 'function') {
      Storage.startPublicPolling(12000);
    }
